...
In particolare, il sito potrebbe essere utilizzato da strutture che dispongono di LIM (lavagne interattive multimediali).

Trattandosi di un sito rivolto ai più piccoli, si è cercato di renderlo simpatico, colorato e accattivante agli occhi di un pubblico infantile soprattutto tramite l'inserimento di molte immagini. Inoltre sono stati utilizzati caratteri dalle dimensioni sufficientemente grandi, per permettere ai bambini di vedere con facilità anche in fase di apprendimento della lettura.
...
Per assicurare un buon grado di uniformità nella visualizzazione dei colori si sono utilizzati esclusivamente colori \inglese{web-safe} vale a dire i colori che nella loro rappresentazione esadecimale contengono solo le cifre \verb+0+, \verb+3+, \verb+6+, \verb+9+, \verb+c+ e \verb+f+ (ripetute).
...
La pagina \sitepage{Mesi.html} contiene una filastrocca sui mesi che presenta un collegamento per ogni mese. La stessa cosa avviene per la pagina \sitepage{Giorni.html}, che contiene però una filastrocca sui giorni, con relativi collegamenti. Si è utilizzata tale tecnica solo con il fine di presentare la pagina in modo simpatico e permettere di muoversi liberamente all'interno delle pagine.

Per rendere più rapida la navigazione si è scelto di offrire la possibilità di saltare gli elementi che esulano dal contenuto in senso stretto, ovvero il menu di navigazione, che si ripete uguale in ogni pagina e che dunque non necessita di essere letto ogni volta.

A tal fine, sono stati inseriti dei collegamenti nascosti (impostando una posizione assoluta che ecceda il margine sinistro), che però risultano accessibili se si legge il solo contenuto testuale della pagina.

Si è inoltre prestata particolare attenzione a evitare la presenza di link circolari e a rendere il testo dei collegamenti facilmente individuabile e, soprattutto, sufficientemente autoesplicativo (evitando accuratamente di utilizzare parole ``vuote'' o deittici come ``clicca qui'').
Inoltre, si è fatta attenzione a non rendere i link presenti nelle \inglese{breadcrumbs}\footnote{%
   \inglese{Breadcrumbs} significa letteralmente ``briciole di pane''. Si tratta di una tecnica utilizzata per aiutare l'utente a non perdersi all'interno del sito, fornendogli le tracce del percorso esplorativo da lui effettuato, proprio come Pollicino aveva lasciato le briciole di pane nel bosco per poter ritrovare la strada di casa.} adiacenti, separandoli tramite il carattere ``>''
...
Infine, è stata predisposta una pagina che visualizza gli errori nel caso in cui la form non sia stata compilata correttamente.

\subsection{Le tabelle}
Le tabelle che compaiono nella sezione \sitepage{Interpreti.html} sono state progettate per essere fruibili anche in forma non visuale, mediante l'utilizzo dell'attributo \texttt{scope} rispettivamente con il valore \texttt{col} per le intestazioni delle colonne e con il valore \texttt{row} per l'intestazione di ogni riga, ovvero il personaggio di cui si elencano gli attori della versione originale e i doppiatori della versione italiana.
...
L'organizzazione delle informazioni nella tabella è resa ancor più chiara dall'attributo \texttt{summary}, in cui oltre a giustificare la suddivisione in righe e colonne è illustrato brevemente il contenuto della tabella, in modo tale che chi accede al contenuto tramite uno \inglese{screen reader} possa saltare il testo qualora non interessato.
...
\subsubsection{Riscontri sui test effettuati con gli screen reader}
...
Per quanto riguarda le tabelle si è controllato che lo \inglese{screen reader} leggesse correttamente le intestazioni di riga e di colonna prima di leggere il contenuto delle celle. Purtroppo tale riscontro non ha avuto esito positivo. Tuttavia, avendo seguito tutte le regole per ottenere tabelle accessibili, si ritiene che l'esito negativo del test derivi dal fatto che gli \inglese{screen reader} utilizzati sono strumenti gratuiti e di conseguenza probabilmente non godono di tutte le funzionalità.

Per quanto riguarda invece le immagini, i test hanno avuto un riscontro positivo. Si è controllato che lo \inglese{screen reader} leggesse il valore degli attributi \texttt{alt} ove presenti. Inoltre si è cercato di capire tramite l'uso di \inglese{screen reader} se il contenuto del sito senza la visione di immagine fosse mantenuto.

Per quanto riguarda la correttezza nella pronuncia di parole appartenenti ad altre lingue, purtroppo il test ha dato esito negativo. Entrambi gli \inglese{screen reader} utilizzati non distinguono il cambiamento di lingua impostato tramite l'attributo  \texttt{xml:lang}.  Anche in questo caso si pensa che la colpa sia del fatto che gli strumenti utilizzati sono gratuiti.
...
L'estensione \progname{Web Developer} per Firefox è stata utilizzata per testare i CSS alternativi a quello principale, e in particolare quello per la stampa. Inoltre, tramite il browser testuale \progname{w3m} è stata testata la navigabilità del sito in modalità solo testo.

Infine, è stato utilizzato lo strumento \progname{IE Tester} per testare la visualizzazione del sito con Internet Explorer a partire dalla versione 5. 
...
I dati conformi a tale schema saranno raccolti nel file XML \texttt{Sondaggi.xml}. Questo file viene usato nelle seguenti occasioni:

\begin{itemize}
	\item dallo pagina \sitepage{FormCreator.html}, nel momento in cui deve essere generata (lato server) la form per l'inserimento dei commenti e per rispondere alle domande del sondaggio;
	\item dalla pagina \sitepage{FormCreator.html}, nel momento in cui devono essere riportati, sotto alla sezione dedicata al form, i commenti inseriti dagli altri visitatori del sito;
	\item dallo script \texttt{addComment.cgi}, nel momento in cui l'utente avrà cliccato il pulsante di submit per inviare al server i dati inseriti nella form. Lo script si occuperà della loro validazione e della loro registrazione in \sitepage{Sondaggi.xml}.
\end{itemize}
...
È la pagina richiamata dal menu principale ed ha il compito di generare lato server la pagina dei sondaggi. Tale scelta è dovuta al fatto che non si desidara perdere gli utenti che utilizzano Google Chrome come browser per la navigazione nel web e non perdere la possibilità di essere indicizzati dai motori di ricerca in quanto non sono in grado di interpretare un file XML. Lo script Perl esegue i seguenti step:
\begin{itemize}
	\item viene recuperato dal disco il file \sitepage{Sondaggi.xml} contenente i dati del sondaggio da proporre agli utenti;
	\item viene recuperato  dal disco il file \texttt{FormCreator.xsl} contenente le regole per trasformare il file XML in un documento HTML visualizzabile da un browser;
...
\begin{itemize}
  \item vengono dapprima definite le librerie di cui lo script necessita;
  ...
  \item se l'inserimento è andato a buon fine allora si procede aggiornando in \sitepage{Sondaggi.xml} il tag <votanti> relativo alle corrispettive <scelte> sulle <domande> a cui ha risposto l'utente;
  \item per ogni domanda si apre il file \sitepage{Sondaggi.xml} e si aggiorna la voce votanti;
  \item quindi si passa alla creazione di un tag <commento> sulla base dei dati: utente, mail, data (il cui valore è creato precedentemente mediante l'uso della funzione \texttt{localtime()}) e il testo stesso del commento;
  \item quindi si termina con l'inserimento del commento e la chiusura del file. Infine lo script reindirizza alla pagina di \sitepage{Conferma.html}.
\end{itemize}
...
Dopo essere emersa la necessità di gestire una mini base di dati, la creazione della medesima è avvenuta seguendo le seguenti fasi/procedure:
\begin{itemize}
	\item identificazione dei requisiti essenziali: capire cosa si intende rappresentare mediante il file xml e quali sono le ``realtà'' che andranno a farne uso;
	\item creazione di un apposito XML Schema atto a rappresentare la realtà evidenziata;
	\item controllo di formattazione del XML Schema;
	\item validazione del medesimo;
	\item creazione di un file \sitepage{Sondaggi.xml} conforme allo schema determinato nei punti precedenti.
\end{itemize}

Come già accennato, il gruppo ha deciso di costruire uno schema con il modello tende alla veneziana, supponendo un futuro riutilizzo dei suoi componenti.

\subsection{Norme di sviluppo CSS}

Il sito basa la gestione del proprio layout grafico su tre fogli di stile CSS: \texttt{NormalLayout.css}, \texttt{PrintLayout.css} e \texttt{SmallLayout.css}. Il primo viene usato per gestire una normale visione su schermo avente una larghezza superiore a \texttt{52em}. Il secondo ha lo scopo di definire la formattazione per la stampa. Il terzo foglio infine si occupa di adattare la pagina nel momento in cui questa raggiunge una larghezza inferiore a \texttt{52em}, e viene caricato sui dispositivi mobile. Nel progettare e sviluppare tali CSS si sono tenute in considerazione le seguenti norme:
...
\begin{itemize}
  ...
  \item Mai abusare degli elementi \texttt{<div>}. È giusto crearli per evidenziare la presenza di una ``regione semantica'' rilevante, o nel caso che il \inglese{layout} grafico richieda particolari effetti flottanti, ma si esorta i componenti a non usarlo la dove possibile.
  \item Nel scegliere i nomi da dare alle classi, tenere in considerazione che tali dovrebbero esprimere il significato dell'oggetto su cui si applicherà, e non le caratteristiche di visualizzazione poiché tali sono mutevoli e potrebbero cambiare in futuro (e.g. se si intende rappresentare una \texttt{<label>} con una certa colorazione, non si dovrà chiamare \texttt{.labelRossa}, ma bensì \texttt{.etichetta}, presupposto il fatto che ci interessa rappresentarla come un etichetta che contraddistingue un determinato \texttt{<input>}).
\end{itemize}
